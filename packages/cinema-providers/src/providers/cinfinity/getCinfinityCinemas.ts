...
export const queryCinfinity = async <T>(
  query: string,
  ...
  try {
    ...
    throw new Error(
      `Cinfinity GraphQL returned invalid JSON (${response.status}): ${responseBody}`,
    );
  }

  if (!response.ok || payload.errors?.length) {
    const errors =
      payload.errors?.map((error) => error.message).join(", ") || responseBody;
    throw new Error(
      `Cinfinity GraphQL request failed (${response.status}): ${errors}`,
    );
  }
...
export const getCinfinityCinemas = async () => {
  const data = await queryCinfinity<{ cinemas: CinfinityCinema[] }>(
    CINFINITY_CINEMAS_QUERY,
  );
