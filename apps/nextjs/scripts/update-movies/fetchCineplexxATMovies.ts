...
export const fetchCineplexxATCatalog = async (): Promise<ProviderCatalog> => {
  ...
  for (const [index, chunk] of cinemaChunks.entries()) {
    console.info(
      `[CineplexxAT][Chunk ${index + 1}/${cinemaChunks.length}] Fetching ${chunk.length} cinemas`,
    );
    const cinemaResults = await Promise.allSettled(
      chunk.map((cinema) =>
        getCineplexxATMovies(cinema.id, cinema.cineplexxAtCinemaId!),
      ),
    );
  ...
  if (failedCinemas.length > 0) {
    throw new Error(
      `Failed to fetch ${failedCinemas.length} CineplexxAT cinemas: ${failedCinemas.join(", ")}`,
    );
  }
