import { ProviderCatalog } from "@waslaeuftin/cinema-providers/server";
...
if (requestedProviderName) {
  ...
  if (providersToUpdate.length === 0) {
    ...
    console.info(
      `Available providers: ${providerFetchers.map((p) => p.name).join(", ")}`,
    );
    process.exit(1);
  }
}
...
try {
  console.info(
    `[Update] Starting update for ${providersToUpdate.length} provider(s)...`,
  );
