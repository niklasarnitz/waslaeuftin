...
export const fetchPremiumKinoCatalog = async (): Promise<ProviderCatalog> => {
  ...
  for (const [index, chunk] of cinemaChunks.entries()) {
    console.info(
      `[PremiumKino][Chunk ${index + 1}/${cinemaChunks.length}] Fetching ${chunk.length} cinemas`,
    );
  ...
  if (failedCinemas.length > 0) {
    throw new Error(
      `Failed to fetch ${failedCinemas.length} PremiumKino cinemas: ${failedCinemas.join(", ")}`,
    );
  }
