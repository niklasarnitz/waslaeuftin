...
console.log(
  `Found ${cineplexxAtCinemas.length} CineplexxAT cinemas to clean up`,
);
...
if (orphanedCities.length > 0) {
  console.log(
    `Deleting ${orphanedCities.length} orphaned Austrian cities: ${orphanedCities.map((c) => c.name).join(", ")}`,
  );
