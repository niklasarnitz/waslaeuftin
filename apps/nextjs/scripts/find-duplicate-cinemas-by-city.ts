...
const main = async () => {
  ...
  if (!Number.isFinite(threshold) || threshold < 0 || threshold > 1) {
    throw new Error(
      "Invalid --threshold value. Please use a number between 0 and 1.",
    );
  }
  ...
  const cinemaIds = cities.flatMap((city) =>
    city.cinemas.map((cinema) => cinema.id),
  );
  ...
  if (cinemaIds.length > 0) {
    ...
    for (const recentShowingCount of recentShowingCounts) {
      recentMoviesCountByCinemaId.set(
        recentShowingCount.cinemaId,
        recentShowingCount._count._all,
      );
    }
  }
  ...
  for (const city of cities) {
    ...
    for (
      let leftIndex = 0;
      leftIndex < cityCinemas.length - 1;
      leftIndex += 1
    ) {
