import { getCinfinityCinemas } from "@waslaeuftin/cinema-providers/server";
...
const cityEntries = await Promise.all(
  Array.from(cityNames).map(
    async (cityName) => [cityName, await upsertCity(cityName)] as const,
  ),
);
...
const createdCinemas = await Promise.all(
  cinemas.map((cinema) => {
    ...
    if (!city) {
      throw new Error(
        `City not found for Cinfinity cinema ${cinema.name}: ${cityName}`,
      );
    }
