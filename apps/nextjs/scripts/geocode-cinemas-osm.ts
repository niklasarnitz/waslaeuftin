...
const scoreCandidate = (
  ...
  let score =
    nameScore * 0.65 + cityScore * 0.25 + Math.min(importance, 1) * 0.1;
...
const parseCliOptions = (args: string[]): CliOptions => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--progress-format" || arg.startsWith("--progress-format=")) {
...
const main = async () => {
  ...
  try {
    ...
    for (const [index, cinema] of cinemas.entries()) {
      ...
      try {
        const nominatimResults = await fetchCandidates(
          cinema.name,
          cinema.city.name,
        );
        ...
        if (!rl) {
          throw new Error(
            "Readline interface not initialized for interactive mode",
          );
        }
