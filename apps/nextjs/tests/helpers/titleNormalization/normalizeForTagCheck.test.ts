...
import { normalizeForTagCheck } from "../../../src/helpers/titleNormalization/normalizeForTagCheck";
...
describe("normalizeForTagCheck", () => {
    ...
    test("handles mixed cases with accents, uppercase, and irregular whitespace", () => {
        expect(normalizeForTagCheck("  L'Amélie \t POULAIN  ")).toBe("l'amelie poulain");
        expect(normalizeForTagCheck("Das   Märchen von \n O")).toBe("das marchen von o");
        expect(normalizeForTagCheck("   Cinépolis \n  VIP   ")).toBe("cinepolis vip");
    });
});
