...
## 2024-08-09 - Accessible Clear Search Buttons
**Learning:** When adding clear buttons inside of search inputs, especially those wrapped in dialogs or comboboxes, users can get frustrated if focus is lost after they clear their search term.
**Action:** Always maintain seamless interaction by ensuring clear buttons are correctly labeled for screen readers (`aria-label`) and immediately return focus to the input (`inputRef.current?.focus()`) upon click.
