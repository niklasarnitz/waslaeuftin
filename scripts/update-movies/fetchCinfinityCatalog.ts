import { getCinfinityMovies } from "@waslaeuftin/cinemaProviders/cinfinity/getCinfinityMovies";
...
export const fetchCinfinityCatalog = async (): Promise<ProviderCatalog> => {
    ...
    const cinfinityCinemas = allCinfinityCinemas.filter((cinema) => isCinemaStale(cinema.lastFetchedAt));
    console.info(`[Cinfinity] Found ${cinfinityCinemas.length} cinemas to fetch (${allCinfinityCinemas.length - cinfinityCinemas.length} skipped, recently fetched)`);

    const cinemaChunks = chunkArray(cinfinityCinemas, CINEMA_BATCH_SIZE);

    for (const [index, chunk] of cinemaChunks.entries()) {
        console.info(`[Cinfinity][Chunk ${index + 1}/${cinemaChunks.length}] Fetching ${chunk.length} cinemas`);
