...
try {
  const kinoTicketsExpressCinemas = await db.cinema.findMany({
    where: { isKinoTicketsExpress: true },
  });
  console.info(
    `[KinoTicketsExpress] Found ${kinoTicketsExpressCinemas.length} cinemas to update`,
  );

  const cinemaChunks = chunkArray(kinoTicketsExpressCinemas, CINEMA_BATCH_SIZE);
  console.info(
    `[KinoTicketsExpress] Processing ${cinemaChunks.length} chunks with up to ${CINEMA_BATCH_SIZE} cinemas each`,
  );

  for (const [index, chunk] of cinemaChunks.entries()) {
    console.info(
      `[KinoTicketsExpress][Chunk ${index + 1}/${cinemaChunks.length}] Fetching provider data for ${chunk.length} cinemas`,
    );
    ...
    const successfulCinemaData: Awaited<
      ReturnType<typeof getKinoTicketsExpressMovies>
    >[] = [];
    ...
    console.info(
      `[KinoTicketsExpress][Chunk ${index + 1}/${cinemaChunks.length}] Fetch finished: ${successfulCinemaData.length} succeeded, ${failedCinemas.length} failed total`,
    );
    ...
    if (successfulCinemaData.length > 0) {
      ...
      console.info(
        `[KinoTicketsExpress][Chunk ${index + 1}/${cinemaChunks.length}] Writing ${movies.length} movies and ${showings.length} showings`,
      );
      ...
      console.info(
        `[KinoTicketsExpress][Chunk ${index + 1}/${cinemaChunks.length}] Write finished`,
      );
    }

    if (index < cinemaChunks.length - 1) {
      console.info(
        `[KinoTicketsExpress][Chunk ${index + 1}/${cinemaChunks.length}] Waiting ${BATCH_DELAY_MS}ms before next chunk`,
      );
  ...
  if (failedCinemas.length > 0) {
    throw new Error(
      `Failed to update ${failedCinemas.length} KinoTicketsExpress cinemas: ${failedCinemas.join(", ")}`,
    );
  }
} finally {
  await db.$disconnect();
